...
function getMatches(term) {
  ...
  appState.directoryContacts.forEach(c => {
    const blob = `${c.name} ${c.category} ${c.phone} ${c.alternatePhone} ${c.address} ${c.website} ${c.defaultAppointmentType} ${c.defaultLocation} ${c.notes}`.toLowerCase();
    if (blob.includes(term)) {
      items.push({ type: "Directory", view: "directory", title: `${c.name || "Contact"}${c.category ? ` - ${c.category}` : ""}`, detail: c.phone || c.address || c.notes || "Directory contact" });
    }
  });
