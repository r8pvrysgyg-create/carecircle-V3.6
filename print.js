...
function getPrintOptions() {
  ...
  const singleDate = document.getElementById("printDate")?.value || todayString();
  const rawStart = document.getElementById("printStartDate")?.value || singleDate;
  const rawEnd = document.getElementById("printEndDate")?.value || rawStart;
...
function getDateLabel(options) {
  if (options.mode === "range") return `${formatDate(options.startDate)} - ${formatDate(options.endDate)}`;
  return formatDate(options.date);
}

function minDate(a, b) {
  return String(a || "9999-99-99") <= String(b || "9999-99-99") ? a : b;
}

function maxDate(a, b) {
  return String(a || "0000-00-00") >= String(b || "0000-00-00") ? a : b;
}
...
function getAppointmentsForPersonAndRange(person, startDate, endDate) {
  return appState.appointments
    .filter(a => a.date >= startDate && a.date <= endDate && a.status !== "Cancelled" && matchesPerson(a.person, person))
...
function getTasksForPersonAndRange(person, startDate, endDate) {
  return appState.tasks
    ...
    .filter(t => !t.dueDate || (t.dueDate >= startDate && t.dueDate <= endDate))
