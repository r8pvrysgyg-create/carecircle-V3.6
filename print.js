import { appState, formatDate, todayString, formatTime, formatDateTimeSeattle } from "./state.js";
    ...
    <article class="daily-print ${forPrint ? "print-mode" : ""}">
      <header class="print-header">
        <div>
          ...
          <p class="print-date">${escapeHtml(formatDate(options.date))}</p>
        </div>
      ...
      <footer class="print-footer">
        CareCircle Daily Summary • ${escapeHtml(formatDate(options.date))}
      </footer>
    </article>
  `;
}
...
function renderPersonSection(person, options) {
  const appointments = getAppointmentsForPersonAndDate(person, options.date);
  const tasks = getTasksForPersonAndDate(person, options.date);
  ...
  return `
    <section class="print-person-section">
      ...
      ${!hasAny ? `<p class="empty-line">No care items found for this date.</p>` : ""}
...
function renderTransportationSection(appointments) {
  ...
  appointments.forEach(a => {
    ...
    if (ret) {
      items.push({
        ...
        label: "Appointment-location pickup",
        time: ret,
        detail: `${a.appointmentType || a.doctor || "Appointment"}${a.driver ? ` • Driver: ${a.driver}` : ""}`
      });
    }
  ...
  if (!items.length) return sectionBlock("Transportation", `<p class="empty-line">No transportation windows today.</p>`);
...
function renderAppointmentsSection(appointments) {
  if (!appointments.length) return sectionBlock("Appointments", `<p class="empty-line">No appointments today.</p>`);
  ...
  return sectionBlock("Appointments", appointments.map(a => {
    ...
    return `
      <div class="print-item">
        <strong>${escapeHtml(appointmentTime)} — ${escapeHtml(a.appointmentType || "Appointment")}</strong>
...
function renderTasksSection(tasks) {
  if (!tasks.length) return sectionBlock("Tasks", `<p class="empty-line">No open tasks due today or earlier.</p>`);
...
function getPrintOptions() {
  return {
    date: document.getElementById("printDate")?.value || todayString(),
...
function getAppointmentsForPersonAndDate(person, date) {
  return appState.appointments
    .filter(a => a.date === date && a.status !== "Cancelled" && matchesPerson(a.person, person))
    .sort((a, b) => String(a.time || "99:99").localeCompare(String(b.time || "99:99")));
}

function getTasksForPersonAndDate(person, date) {
...
function getPrintStyles() {
  return `
    ...
    .print-row { display: grid; grid-template-columns: 120px 1fr; gap: 10px; }
    .print-row small { grid-column: 2; color: #475569; }
    .check-line { display: flex; gap: 10px; align-items: center; }
    .box { display: inline-block; width: 14px; height: 14px; border: 2px solid #111827; }
    .empty-line { color: #64748b; font-style: italic; }
    .print-footer { margin-top: 20px; padding-top: 10px; border-top: 1px solid #cbd5e1; font-size: 12px; color: #64748b; text-align: center; }
